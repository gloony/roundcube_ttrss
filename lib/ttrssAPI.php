 ...
 * https://github.com/gloony/tt-rss-api-php-class
 * from: https://github.com/tofika/tt-rss-api-php-class
 *
 * @author Anatoliy Kultenko "tofik"
 * @author David Chardonnens "gloony"
...
class ttrssAPI
{
    ...
    public function getHeadlines( $feed_id, $limit, $page = 1, $is_cat = 'false', $show_excerpt = 'true', $show_content = 'true', $view_mode = 'unread', $order_by = 'feed_dates')
    {
    ...
    public function unsubscribeFeed( $feed_id)
    {
        $params = array( "sid" => $this -> t_session_id, "op" => "unsubscribeFeed", "feed_id" => $feed_id);
        $params = json_encode( $params);
        $response = $this -> t_api_query( $this->t_api_url, $params);
        if ( $response['code'] == 200) {
            $tarray = json_decode( $response['text'], true);
